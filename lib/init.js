...
function Order(dependency) {
  if (!(this instanceof Order)) {
    return new Order(dependency);
  }
  this.dependency = dependency;
}
init.Order = Order;

function isNotOrder(value) {
...
function getParameter(resources, parameter) {
  if (_.isString(parameter)) {
    ...
    var retrieveParamter = getParameter.bind(null, resources);
    return parameter.filter(isNotOrder).map(retrieveParamter);
  } else if (isOnlyObject(parameter)) {
    var retrieveParamater = getParameter.bind(null, resources);
    var values = _.values(parameter).filter(isNotOrder).map(retrieveParamater);
